...
form.addEventListener("submit", e => {
    ...
    let warnings = ""
    let regexgrupo = /[0-9][A-Z][A-Z][0-9]/
    ...
    if (anios < 0) {
        warnings += `No puede ser menor a 0 años <br>`
        entrar = true
    }
    ...
    if (grupo.value.length < 1 || !regexgrupo.test(grupo.value)) {
        warnings += `El grupo esta vacio o no es valido <br>`
        entrar = true
    }
    ...
    if (entrar) {
        ...
        warnings += 'Hola '
        warnings += dnombres.value
        warnings += ' '
        warnings += dape1.value
        warnings += ' '
        warnings += dape2.value
        warnings += ' '
        warnings += ', verifica que los datos que ingresaste sean <br>'
        warnings += 'correctos: <br>'

        warnings += 'DATOS DEL NIÑO O DE LA NIÑA: <br>'
        warnings += 'Nombre Cendi: '
        warnings += cendi.value
        warnings += '<br>'
        warnings += 'Folio (boleta): '
        warnings += boleta.value
        warnings += '<br>'
        warnings += 'Grupo: '
        warnings += grupo.value
        warnings += '<br>'
        warnings += 'Nombres niño: '
        warnings += nombres.value
        warnings += '<br>'
        ...
        warnings += 'DATOS DEL DERECHOHABIENTE: <br>'
        warnings += 'Nombres : '
        warnings += dnombres.value
        warnings += '<br>'
        warnings += 'Apellido paterno: '
        ...
        warnings += '<br>'
        warnings += 'Calle : '
        warnings += dcalle.value
        warnings += '<br>'
        warnings += 'Colonia: '
        warnings += dcolonia.value
        warnings += '<br>'
        warnings += 'Numero exterior: '
        warnings += dext.value
        warnings += '<br>'
        warnings += 'Numero interior: '
        warnings += dint.value
        warnings += '<br>'
        warnings += 'Alcaldia: '
        warnings += dalcaldia.value
        warnings += '<br>'
        warnings += 'C.P. : '
        warnings += dcp.value
        warnings += '<br>'
        warnings += 'Telefono fijo: '
        warnings += dtelfijo.value
        warnings += '<br>'
        warnings += 'Telefono celular : '
        warnings += dtelcel.value
        warnings += '<br>'
        warnings += 'Correo : '
        warnings += dcorreo.value
        warnings += '<br>'
        warnings += 'Estado: '
        warnings += estado.value
        warnings += '<br>'
        warnings += 'Ocupacion: '
        warnings += docupacion.value
        warnings += '<br>'
        warnings += 'CURP: '
        warnings += dcurp.value
        warnings += '<br>'
        warnings += 'Plaza: '
        warnings += dplaza.value
        warnings += '<br>'
        warnings += 'Sueldo : '
        warnings += dsueldo.value
        warnings += '<br>'
        warnings += 'Numero empleado : '
        warnings += dnumempleado.value
        warnings += '<br>'
        warnings += 'Adscripcion: '
        warnings += dadscripcion.value
        warnings += '<br>'
        warnings += 'Horario de trabajo: '
        warnings += dhorariotrabajo.value
        warnings += '<br>'
        warnings += 'Nombre de cargo: '
        warnings += dnombre_cargo.value
        warnings += '<br>'
        warnings += 'Extension: '
        warnings += dextension.value
        warnings += '<br>'

        if (selectedSize === "si") {
            warnings += 'DATOS DEL CONYUJE: <br>'
            warnings += 'Nombres : '
            warnings += cnombres.value
            warnings += '<br>'
            warnings += 'Apellido paterno: '
            warnings += cape1.value
            warnings += '<br>'
            warnings += 'Apellido materno: '
            warnings += cape2.value
            warnings += '<br>'
            warnings += 'Calle : '
            warnings += ccalle.value
            warnings += '<br>'
            warnings += 'Colonia: '
            warnings += ccolonia.value
            warnings += '<br>'
            warnings += 'Numero exterior: '
            warnings += cext.value
            warnings += '<br>'
            ...
            warnings += '<br>'
            warnings += 'Alcaldia: '
            warnings += calcaldia.value
            warnings += '<br>'
            warnings += 'C.P. : '
            warnings += ccp.value
            warnings += '<br>'
            warnings += 'Telefono fijo: '
            warnings += ctelfijo.value
            warnings += '<br>'
            warnings += 'Telefono celular : '
            warnings += ctelcel.value
            warnings += '<br>'
            warnings += 'Lugar de trabajo: '
            warnings += clugartrabajo.value
            warnings += '<br>'
            warnings += 'Estado: '
            warnings += cestado.value
            warnings += '<br>'
            warnings += 'Ocupacion: '
            warnings += cocupacion.value
            warnings += '<br>'
            warnings += 'Domicilio trabajo: '
            warnings += comtrabajo.value
            warnings += '<br>'
            warnings += 'Telefono trabajo : '
            warnings += cteltrabajo.value
            warnings += '<br>'
            warnings += 'Extension: '
            warnings += cextension.value
            warnings += '<br>'
            warnings += 'Religion: '
            warnings += creligion.value
            warnings += '<br>'
            warnings += 'Fecha archivo : '
            warnings += fechaarch.value
            warnings += '<br>'
        }
        parrafo.innerHTML = warnings
    }
